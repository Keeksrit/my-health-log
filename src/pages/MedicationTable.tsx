...
import { fetchLogsForSchedule, upsertLog, bumpTime } from '../lib/medication'
...
export default function MedicationTable({ schedule, onBack }: Props) {
  ...
  async function handleTimeBump(log: MedicationLog, minutes: number) {
    const newTime = bumpTime(log.time, minutes)
    await handleChange(log, 'time', newTime)
  }
              ...
              <div className={styles.colTime}>
                <div className={styles.timeControl}>
                  <button
                    ...
                    onClick={() => handleTimeBump(log, -15)}
                  >−</button>
                  <span className={styles.timeVal}>{log.time}</span>
                  <button
                    className={styles.timeStepBtn}
                    onClick={() => handleTimeBump(log, 15)}
                  >+</button>
